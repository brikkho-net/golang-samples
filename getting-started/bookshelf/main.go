...
func (b *Bookshelf) appErrorf(r *http.Request, err error, format string, v ...interface{}) *appError {
	...
	b.errorClient.Report(errorreporting.Entry{
		Error: err,
		Req:   r,
	})
	...
	b.errLogger.Printf("Handler error: status code: %d, message: %s, underlying err: %#v", appErr.Code, appErr.Message, appErr.Error)
